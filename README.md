...
For the format of the data, I will compile the most popular legends and cryptids into a CSV file and convert it into a point-based Shapefile. I would also like to give each of these creatues their own little unique illustration as a point marker on the final map, but this depends on how much time I have to create these illustrations. I may just end up going with color-coded markers to indicate myth groupings (i.e. cryptids, events, etc. depending on the final dataset). 

## 2. Topic and Geographic Phenomena to Explore

For this final project, I want to map a set of popular cryptids and legends in Kentucky. Since my undergraduate years, I have always wanted to create a comprehensive map of local legends, myths, cryptids, and legendary figures for the whole world. Although that scope is impossible for the time that I have for this final project, I want to give it a start at Kentucky first then perhaps expand in the future. In terms of geography, the map will represent Kentucky, with relevant major geographical features (rivers, lakes, tree cover) also being shown. Below are my tentative title and subtitle:

**Tentative Title:** The Legends and Myths of Kentucky <br>
**Tentative Subtitle:** What Lurks in The Bluegrass State?

Here are my responses for the other provided questions:
...
Realizing this map in the browser will be a bit tricky, as I want to include functionality for users to add their own legends, myths, and cryptids that they know of. Here are my responses to the provided questions:
...
2. QGIS: for the processing of the CSV data into a GeoJSON for ease of use

**Alternatively, I may use geojson.io to create a ready GeoJSON of all the data in lieu of QGIS

**Format of Data Storage**
All map data will be stored as a GeoJSON. I am not quite sure about the storage of the images and their matching descriptions though since I also want the user to be able to contribute.

**Anticipated JS (and CSS) Libraries**
1. Leaflet: for drawing the basemap and adding markers and popups
2. Bootleg: for design and style functionality 
